...
- その直後に、**「ひとこと」を再生**（天気予報は任意機能。既定では無効）
...
| おまけ | 時報のあとに「ひとこと」を再生。天気予報は任意機能（既定では無効、設定で有効化可） |
...
| 天気予報を有効にする | 既定は無効。手順は [docs/SETUP.md](docs/SETUP.md)「天気予報を有効にする方法」参照 |
