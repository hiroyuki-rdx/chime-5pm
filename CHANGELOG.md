...
- **事前生成音声**: VOICEVOX（ずんだもん・話者 3）で作り置きした読み上げ音声 67 件を `assets/voice/` に同梱した
  - 時刻アナウンス 7 件とひとこと 57 件をすべて賄うため、Pi 上では実行時の音声合成が発生しない
  - Pi に VOICEVOX を導入する必要はない（`prerecorded` エンジンが WAV を再生するだけ）
