...
python3 campus_chime.py --weather          # 天気予報
...
| `天気予報を取得できませんでした` | ネットワーク断 | 正常動作（ひとことに切替） |
...
取得結果は 60 分キャッシュされます（気象庁への負荷を避けるため）。すぐに反映したい場合はサービスを再起動してください。
...
天気予報以外は Wi-Fi なしでも動作するため、放送そのものは止まりません。
