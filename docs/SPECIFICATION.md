...
終了コード: `0` 正常 / `1` 実行時エラー（`--weather` での天気取得失敗、`--generate-assets` での音声合成失敗、`--say`・`--test-hourly`・`--test`・`--test-all` で再生可能なセグメントを 1 つも用意できなかった場合。時報音は合成不要のため必ず鳴り、おまけの読み上げのみが失敗した場合は `0` のまま。おまけの天気予報取得に失敗しても「ひとこと」への切り替えに成功していればエラー扱いしない）/ `2` 引数・設定エラー（`--say` に空文字列や空白のみを渡した場合を含む）。
