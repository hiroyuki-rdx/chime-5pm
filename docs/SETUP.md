...
滋賀県にはもう一つ「北部」（彦根・長浜・米原・高島など）という一次細分区域があります。北部に切り替えるには、天気・降水確率用の `area_name` を `"北部"` に、気温用の `temp_area_name` を観測地点名の `"彦根"` に変更します（この 2 つは気象庁 JSON 上で体系が異なるため、別々に指定する必要があります）。

```json
{ "weather": {
    "jma": { "area_name": "北部", "temp_area_name": "彦根", "label": "滋賀（北部）" } } }
```

他の都道府県に変える場合は、気象庁の地域コードを <https://www.jma.go.jp/bosai/common/const/area.json> の `offices` から探します（例: 大阪府 `270000`、愛知県 `230000`、福岡県 `400000`）。
...
いずれの地域コード・地域名も本リポジトリでは実通信で検証していません。設定を変えたら、必ず実機で `python3 campus_chime.py --weather` を実行し、読み上げ文が意図どおりになるか確認してください。
